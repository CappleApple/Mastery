...
public final class VisualScriptScreen extends Screen {
    ...
    private int page;
    private int keywordEvent;
    private boolean threshold(){return keywordEvent==1;}
    ...
    private String actionsKey() { return keyword()?switch(keywordEvent){case 1->"threshold_actions";case 2->"stacks_lost_actions";case 3->"all_stacks_lost_actions";default->"tick_actions";}:"actions"; }
    ...
    @Override protected void init() {
        ...
        if(keyword())addRenderableWidget(Button.builder(Component.literal(EditorSchema.label(actionsKey())),b->{keywordEvent=(keywordEvent+1)%4;page=0;rebuildWidgets();}).bounds(110,65,Math.min(150,width-234),20).build());
        addRenderableWidget(Button.builder(Component.literal("JSON editor"),b->owner.openJson()).bounds(width-118,65,104,20).build());
        String event=keyword()?switch(keywordEvent){case 1->"Stack threshold";case 2->"Stacks lost";case 3->"All stacks lost";default->"Periodic tick";}:draft.has("event")?draft.get("event").getAsString():"hit";
        addRenderableWidget(Button.builder(Component.literal(EditorSchema.label(event)),b->{
            if(keyword()){if(keywordEvent>1)minecraft.setScreen(new VisualDefinitionScreen(owner,this,draft,""));else openValue(threshold()?"threshold":"tick_interval",new JsonPrimitive(threshold()?5:20));}
        ...
        if(!keyword())addRenderableWidget(Button.builder(Component.literal("+ Condition"),b->minecraft.setScreen(new EditorChoiceScreen(this,"Condition type",java.util.List.of("health","keyword","damage"),v->{pending=ScriptSchema.condition(v);pendingPath="conditions";conditions.add(pending);}))).bounds(conditionsX+5,height-78,column-10,20).build());
    ...
    private void card(JsonArray list,int index,int x,int y,String path,boolean action) {
        ...
        if(!action&&type.equals("damage"))target="with matching damage";
    ...
    @Override public void render(GuiGraphics g,int x,int y,float tick) {
        ...
        if(keyword())g.drawWordWrap(font,Component.literal(keywordEvent>1?"Runs after stack removal. Per-stack actions use the number of stacks lost.":threshold()?"Reaching the threshold runs this chain. Optional consumption removes the threshold stacks.":"Each interval runs this chain while the keyword remains active. Damage can scale with stacks."),left+column+18,top+10,column-16,0xC5CFDC);
        else if(list("conditions").isEmpty())g.drawWordWrap(font,Component.literal("No conditions: every matching event is eligible. Add damage, health, or keyword checks."),left+column+18,top+10,column-16,0xC5CFDC);
