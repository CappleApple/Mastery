...
public final class ChargeService {
    private static final class Charge {
        String spell;int slot,levels,base;double strength=-1;long started,released=-1;boolean completed;
    ...
    public static void prepare(ServerPlayer player,String spell,int slot) {
        ...
        if(charge.enabled()&&SpellRegistry.getSpell(spell).getCastType()!=CastType.CONTINUOUS)
            CHARGES.put(player.getUUID(),new Charge(spell,slot,stackedLevels(player,spell),player.level().getGameTime(),charge));
    }
    ...
    public static int nativeLevel(ServerPlayer player,String spell,int original) {
        ...
        return (int)Math.clamp(original+(long)Math.floor(strength(player,charge)*charge.options.spellLevelsPerStage()),1,Integer.MAX_VALUE);
    }
