...
Active spell nodes display the same localized guide description used by Iron's Scroll Forge in their details panel. Stat tooltips use native attribute names and percentage formatting.
...
Base native spell levels remain independent of charge ranks. `level` defines the binding's starting native level; an `unlock_spell` effect can supply a higher rank-based starting level, and `spell_level` modifiers add further levels. Optional `spell_levels_per_stage` adds a charged effect-level bonus without a second mana charge.
...
A modifier node targets a `spell`, sets `modifier` to `mastery:native_spell`, and includes a `mastery:spell_modifier` effect:
