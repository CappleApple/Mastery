...
def generate():
    ...
    def node(id,tree,name,effects,description,ranks=1,parent=None):
        ...
        if id in ('fire/weapon_aspect', 'fire/conversion'):
            tree='flame_blade'
            if id=='fire/weapon_aspect':parent='mastery:spellblade_practice'
        value=dict(tree='mastery:'+tree,name=name,description=description,icon='minecraft:book',type='modifier' if any(effect.get('type')=='mastery:trigger' for effect in effects) else 'passive',cost=1,max_rank=ranks,level=0,dependencies=[parent or 'mastery:'+tree+'/foundation'],effects=effects,visibility='available',toggleable=True)
        if id in ('fire/scorch','lightning/thunder'):
            spell='fireball' if id=='fire/scorch' else 'lightning_bolt'
            value.update(spell='irons_spellbooks:'+spell,modifier='mastery:native_spell',dependencies=['mastery:'+spell])
    ...
    node('fire/scorch','fire','Scorch',[dict(type='mastery:trigger',trigger='mastery:scorch_on_hit')],'Fireball hits have a 25% chance to apply Scorch.')
    node('lightning/thunder','lightning','Gathering Thunder',[dict(type='mastery:trigger',trigger='mastery:thunder_on_hit')],'Lightning Bolt hits build Thunder. Full stacks strike nearby enemies.')
